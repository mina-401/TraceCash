...
# TraceCash — 개인 고정지출 관리 플랫폼 (백엔드)

> "매달 자동으로 빠져나가는 모든 것"을 한 곳에서 관리하는 고정지출 추적 서비스의 REST API 서버입니다.
...
TraceCash는 넷플릭스·월세·통신비처럼 **매달 반복적으로 발생하는 고정지출**을 관리하는 백엔드 서비스입니다.
...
스케줄러는 10초(개발) 또는 매일 새벽 3시(운영)마다 실행됩니다. 서버 재시작이나 중복 실행에도 **동일한 인스턴스가 중복 생성되지 않습니다.**
...
조회 시 공용 + 내 커스텀을 함께 반환합니다 (`OR [{ userId: null }, { userId }]`).
...
| PATCH | /recurring-expenses/:id | 수정 | * |
...
3. 토큰 유효기간: 1일
4. 토큰 만료 시 401 → 재로그인 필요
```

비밀번호는 bcrypt(salt rounds: 10)로 단방향 해싱하여 저장합니다. 평문은 절대 저장되지 않습니다.

보안상 "이메일 없음"과 "비밀번호 틀림"은 동일한 메시지("이메일 또는 비밀번호가 올바르지 않습니다")를 반환합니다. 이메일 존재 여부를 외부에 노출하지 않기 위함입니다.
